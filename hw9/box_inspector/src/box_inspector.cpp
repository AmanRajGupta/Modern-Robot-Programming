...
//ADDED THIS ------------------------------------------------------------------------------------------------------------------------------------------------------------
    box_camera_2_subscriber_ = nh_.subscribe("/ariac/box_camera_2", 1, &BoxInspector::box_camera_2_callback, this);
//-----------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
//ADDED THIS -----------------------------------------------------------------------------------------------------------------------------------------------------
void BoxInspector::box_camera_2_callback(const osrf_gear::LogicalCameraImage::ConstPtr & image_msg) {
    if (!got_new_snapshot_2_) {
        box_inspector_image_2_ = *image_msg; //copy the current message to a member data var, i.e. freeze the snapshot
        got_new_snapshot_2_ = true;
        //ROS_INFO_STREAM("received box-camera image of: " << box_inspector_image_2_ << endl);
        int n_models_2 = box_inspector_image_2_.models.size();
        //ROS_INFO("%d models seen ", n_models_2);
    }
}
...
bool BoxInspector::get_new_snapshot_from_box_cam_2() {
    got_new_snapshot_2_ = false; //change variable name to got_new_snapshot_2_
    double dt = 0.05;
    double timer = 0.0;
    while (!got_new_snapshot_2_ && (timer < BOX_INSPECTOR_TIMEOUT)) { //update the variable name
        ros::spinOnce();
        ros::Duration(dt).sleep();
        timer += dt;
    }
...
//averages multiple snapshots; returns a LogicalCameraImage with coordinates wrt camera frame
bool BoxInspector::get_filtered_snapshots_from_box_cam_2(osrf_gear::LogicalCameraImage &filtered_box_camera_image) {
    int n_snapshots = 3; //choose to average this many snapshots
    vector<geometry_msgs::Pose> sum_poses, averaged_poses;
    //osrf_gear::LogicalCameraImage filtered_box_camera_image;

    if (!get_new_snapshot_from_box_cam_2()) {
        return false;
    } //failed to get new image; blackout?

    //got at least one snapshot; proceed to average
    int num_parts_seen = box_inspector_image_2_.models.size();
    averaged_poses.resize(num_parts_seen);
    sum_poses.resize(num_parts_seen);
    ...
    for (int i = 0; i < num_parts_seen; i++) { //initialize, based on first successful snapshot
        sum_poses[i] = box_inspector_image_2_.models[i].pose;
    }

    int i_snapshots = 1; //count how many good snapshots are to be included in  average
    for (int i = 0; i < n_snapshots; i++) { // try for this many more snapshots
        if (get_new_snapshot_from_box_cam_2()) {
            //filtered_box_camera_image = box_inspector_image_2_;
            if (box_inspector_image_2_.models.size() == num_parts_seen) { //if here, got a new snapshot consistent w/ first snapshot;
                //start  averaging process
                i_snapshots++;
                for (int j = 0; j < num_parts_seen; j++) {
                    sum_poses[j].position.x += box_inspector_image_2_.models[j].pose.position.x;
                    sum_poses[j].position.y += box_inspector_image_2_.models[j].pose.position.y;
                    sum_poses[j].position.z += box_inspector_image_2_.models[j].pose.position.z;
                    sum_poses[j].orientation.x += box_inspector_image_2_.models[j].pose.orientation.x;
                    sum_poses[j].orientation.y += box_inspector_image_2_.models[j].pose.orientation.y;
                    sum_poses[j].orientation.z += box_inspector_image_2_.models[j].pose.orientation.z;
                    sum_poses[j].orientation.w += box_inspector_image_2_.models[j].pose.orientation.w;
    ...
    //put these poses into a logical-camera image message:
    filtered_box_camera_image = box_inspector_image_2_; //this sets dimension, part  names,  and camera pose
    //NOTE: all  coords are w/rt box camera frame
    for (int i = 0; i < num_parts_seen; i++) {
        filtered_box_camera_image.models[i].pose = averaged_poses[i];
    }
...
//as above, but with more args for org by part indices
bool BoxInspector::update_inspection_2(vector<osrf_gear::Model> desired_models_wrt_world,
        vector<osrf_gear::Model> &satisfied_models_wrt_world,
        vector<osrf_gear::Model> &misplaced_models_actual_coords_wrt_world,
        vector<osrf_gear::Model> &misplaced_models_desired_coords_wrt_world,
        vector<osrf_gear::Model> &missing_models_wrt_world,
    ...
    //if blackout, DO NOT clear the model vectors!
    if (!get_filtered_snapshots_from_box_cam_2(filtered_box_camera_image)) {			//basically only changed this from cam to cam_2
        return false;
    }
    ...
    inventory_msgs::Part bad_part;
    string model_name;
    if (get_bad_part_Q2(bad_part)) {							//changed from Q1 to Q2
        //found a bad part; match it to filtered image and classify it as orphaned
        bool found = false;
        int ipart_seen = 0;
        string bad_part_name(bad_part.name);
...
//added this ---------------------------------------------------------------------------------------------------------------------------------------
bool BoxInspector::get_box_pose_wrt_world_2(geometry_msgs::PoseStamped &box_pose_wrt_world) {
    geometry_msgs::Pose cam_pose, box_pose; //cam_pose is w/rt world, but box_pose is w/rt camera
    //default: assign assumed box pose until/unless get camera data
    ...
    //get a new (filtered) snapshot of the box-inspection camera:
    osrf_gear::LogicalCameraImage filtered_box_camera_image;
    if (!get_filtered_snapshots_from_box_cam_2(filtered_box_camera_image)) {
        return false;
    }

    //ROS_INFO("got box-inspection camera snapshot");
    //look for box in model list:
    int num_models = box_inspector_image_2_.models.size(); //how many models did the camera see?
    if (num_models == 0) return false;
    string box_name("shipping_box"); //does a model match this name?
    osrf_gear::Model model;
    cam_pose = box_inspector_image_2_.pose;
    ROS_INFO("box cam sees %d models", num_models);
    for (int imodel = 0; imodel < num_models; imodel++) {
        model = box_inspector_image_2_.models[imodel];
        string model_name(model.type);
        if (model_name == box_name) {
            box_pose = model.pose;
            ROS_INFO_STREAM("get_box_pose_wrt_world(): found box at pose " << box_pose << endl);
