...
bool BoxInspector2::get_new_snapshot_from_box_cam(int cam_num) {
    ...
    switch(cam_num) {
		...
		ros::spinOnce();			//added these three lines
                ros::Duration(dt).sleep();
                timer += dt;
                ROS_WARN("Trying to grab a snapshot from box camera 2");
            }
            if (timer >= BOX_INSPECTOR_TIMEOUT) {
...
bool BoxInspector2::get_filtered_snapshots_from_box_cam2(osrf_gear::LogicalCameraImage &filtered_box_camera_image) {
    ...
    switch(cam_num) {
        case CAM1:
            ...
            break;
        case CAM2:
	    box_inspector_image=box_inspector_image2_; 	//added this to get image from cam 2
            ROS_WARN("trying to store snapshot from box camera 2");
            break;     
        default:
