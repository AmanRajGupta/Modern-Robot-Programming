...
float g_x_object;
float g_y_object;
bool g_got_new_value=false;

void block_callback(const geometry_msgs::PoseStamped& message_holder) 
{ 

	g_x_object = message_holder.pose.position.x;
        g_y_object = message_holder.pose.position.y;
	g_got_new_value = true;
} 
...
//added in -------------------------------------------------------------------------------------------------
    ros::Subscriber block_pose_subscriber = nh.subscribe("block_pose", 1, block_callback);

	while(!g_got_new_value){  //spins until it gets a new value
		ros::spinOnce(); 
	}

	ROS_INFO("Got the object position!");
    ...
    O_des << g_x_object , g_y_object , 0.3; //desired position is x, y , and any z that is above the object
    ...
    if (rtn_val == arm_motion_action::arm_interfaceResult::SUCCESS) {
        ...
        //tool_pose.pose.position.y= g_y_object; //move object to correct y value 
        tool_pose.pose.position.z = 0.05; //0.01;  --hover above the object which is at 0.035 = z for its height         
        ...
        ROS_INFO("requesting plan to depart with grasped object after moving to (0.5,0.5):");
        tool_pose.pose.position.x = 0.5;         //desired location to put x coordinate for block
        tool_pose.pose.position.y = 0.5;         //desired location to put y coordinate for block
	tool_pose.pose.position.z = 0.3;         
        ...
        ROS_INFO("descending at (0.5,0.5) to place object:");

	tool_pose.pose.position.z = 0.0343;         
      ...
      ROS_INFO("moving out of the way of block and camera (0.3,0.3,0.3)");
    ...
    return 0;
}	//try 0.4, 0.4 starting position for the object, it seems more accurate of a collision
