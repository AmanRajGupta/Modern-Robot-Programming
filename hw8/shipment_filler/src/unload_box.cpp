...
int main(int argc, char** argv) {
    ...
    while (boxInspector.get_bad_part_Q1(current_part)) { //was originally an if
	...
	model_to_part(orphan_models_wrt_world[min_i], current_part, inventory_msgs::Part::QUALITY_SENSOR_1); //fills value in for current part
    	status = robotBehaviorInterface.pick_part_from_box(current_part); //discards current part
    ...
    for (int i=0;i<nparts;i++) {
       ROS_INFO_STREAM("orphaned  parts: "<<orphan_models_wrt_world[i]<<endl);
	ROS_INFO("Removing Orphaned Parts");
	model_to_part(orphan_models_wrt_world[i], current_part, inventory_msgs::Part::QUALITY_SENSOR_1);	//sets new model name for current part
    	status = robotBehaviorInterface.pick_part_from_box(current_part);	//discards current part
    }
